...
The purpose of this note is to show how **Bayesian regression** can be used to provide a credibility weighted approach to the analysis of mortality experience.
...
Ideally, any such adjustment should be done considering all basis dimensions simultaneously. Through the use of Bayesian regression (as opposed to a "vanilla" GLM regression), we can obtain credibility weighted estimates that:
...
Noting that $\lambda$ has now been written as an exponential transformation of a linear combination of parameters.

Fitting a Poisson distribution is readily possible within the GLM framework, however the exponential transformation above requires that we use a "log-link" function; this is more of an implementation concern and will not be discussed further. It is worth noting though that this transformation ensures that the fitted value for $\lambda$ will always be non-negative, as required for the Poisson distribution.
